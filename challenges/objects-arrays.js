...
for(let i = 0; i < graduates.length; i++) {
  let filter = graduates[i].university;
  universities.push(filter);
}
...
const lowerCase = zooAnimals.map(aName => `LowerCase Name: ${aName.animal_name}`.toLowerCase());
...
largerPopulation.push(zooAnimals.filter(lowPop => lowPop.population < 5));
...
*/
const populationTotal = zooAnimals.reduce(function (acc, obj) {
